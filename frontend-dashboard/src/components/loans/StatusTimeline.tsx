                ...
                <div>
                  <span
                    className={clsx(
                      ...
                      CANDIDATE_STATUS_CHART_COLORS[event.new_status] ||
                      'bg-gray-500'
                    )}
                  >
                    {event.new_status === 'qualified' && (
                      <svg className="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                      </svg>
                    )}
                    {(event.new_status === 'hard_disq' ||
                      event.new_status === 'soft_disq') && (
                      <svg className="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                      </svg>
                    )}
                    {!['qualified', 'hard_disq', 'soft_disq'].includes(
                      event.new_status
                    ) && (
                      ...
                      {event.previous_status ? (
                        <>
                          <span className="font-medium">
                            {CANDIDATE_STATUS_LABELS[event.previous_status] ??
                              event.previous_status}
                          </span>
                          {' → '}
                          <span className="font-medium">
                            {CANDIDATE_STATUS_LABELS[event.new_status] ??
                              event.new_status}
                          </span>
                        </>
                      ) : (
